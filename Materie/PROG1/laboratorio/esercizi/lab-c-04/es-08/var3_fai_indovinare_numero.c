...
int main(void)
{
	...
	while(scanf("%d",&t) && !trovato && numTentativi < MAX_TENTATIVI){
		if (t == numero){
			printf("\nNumero trovato! Numero tentativi: %d", numTentativi);
			trovato = true;
		} else if(t > numero){
			printf("\nIl numero scelto e' maggiore del numero da indovinare. Inseriscine un altro: ");
			numTentativi++;
		} else {
			printf("\nIl numero scelto e' minore del numero da indovinare. Inseriscine un altro: ");
			numTentativi++;
		}

	} 

	if (numTentativi == MAX_TENTATIVI){
		printf("Hai superato il numero di tentativi massimo (%d)! Il numero era: %d.", MAX_TENTATIVI, numero);
	}
